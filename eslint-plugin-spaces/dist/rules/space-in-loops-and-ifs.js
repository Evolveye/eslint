    ...
    create(context, [option]) {
        ...
        return {
            IfStatement(node) {
                ...
                checkSpaces({ context, code: sourceCode, insertSpaces }, tokens, node.test, 1);
            },
            ForOfStatement(node) {
                const tokens = sourceCode.getTokens(node);
                ...
                checkSpaces({ context, code: sourceCode, insertSpaces }, tokens, node.right, firstParenIndex);
            },
            ForInStatement(node) {
                const tokens = sourceCode.getTokens(node);
                checkSpaces({ context, code: sourceCode, insertSpaces }, tokens, node.right, 1);
            },
            ForStatement(node) {
                if (node.update === null)
                    return;
                ...
                if (firstParenIndex === undefined)
                    return;
                checkSpaces({ context, code: sourceCode, insertSpaces }, tokens, node.update, firstParenIndex);
            },
            WhileStatement(node) {
                checkSpaces({ context, code: sourceCode, insertSpaces }, sourceCode.getTokens(node), node.test, 1);
            },
        };
    },
});
